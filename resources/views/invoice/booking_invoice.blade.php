...
<head>
    ...
    <meta http-equiv="X-UA-Compatible" content="ie=edge">
    <title> {{ __('Booking Invoice - RentBD') }} </title>
...
<div class="invoice-area">
    <div class="invoice-wrapper">
        <div class="invoice-header">
            <div class="invoice-flex-contents">
                <div class="invoice-logo">
                    <img src="{{ public_path('img/logo.jpg') }}" style="margin-top: 15px; width: 180px; height: 120px;" alt="Company Logo">
                </div>
            </div>
        </div>
        @foreach($data as $key)
            ...
            <div class="invoice-details-flex">
                <div class="invoice-single-details">
                    ...
                    <ul class="details-list">
                        <li class="list"> {{ Auth::user()->name }} </li>
                        <li class="list"> +88{{ Auth::user()->phone }} </li>
                        <li class="list"> {{ Auth::user()->email }} </li>
                    </ul>
                </div>
                <div class="invoice-single-details" style="float:right;margin-top:-120px;">
                    ...
                    <h4 class="invoice-details-title">{{ __('Bill From:') }}</h4>
                    <ul class="details-list">
                        <li class="list"> <strong>{{ __('Name') }}: </strong> {{ $key->name }} </li>
                        <li class="list"> <strong>{{ __('Floor') }}: </strong> {{ $key->floor }} </li>
                        <li class="list"> <strong>{{ __('Holding') }}: </strong> {{ $key->holding }} </li>
                        <li class="list"> <strong>{{ __('Address') }}: </strong>{{ $key->road }}/{{ $key->area }}/{{ $key->district }} </li>
                    </ul>
                </div>
            </div>
        </div>
        ...
        <div class="item-description">
            <h5 class="table-title">{{ __('Booking Details') }}</h5>
        ...
        <footer>
            <table>
                <tr>
                    <td>
                        <br>
                        <span>Owner's Signature</span>
                        <br>
                        <img class="sign" src="{{ $key->sign }}" width="150px" height="50px" alt="sign" style=" border: 1px;border-style: solid;border-color: black;">
                    </td>
                </tr>
            </table>
